...
class Variable implements VariableInterface
{
    ...
    )
    {
        ...
        if(!$this->undetermined) {
            $this->update($value, reference:$reference, mask:$mask);
        }
    }
    ...
    public function assign(
        mixed $value, ?bool $reference=null, ?NDArray $mask=null
    ) : void
    {
        $this->update($value, reference:$reference, mask:$mask);
    }
        ...
        mixed $value, ?bool $reference=null, ?NDArray $mask=null) : void
    {
        ...
        if($value instanceof NDArray) {
            if($reference) {
                ...
                // Copying NDArray before translate from NDArray to NDArrayCL
                // when Backend is OpenCL.
                $newValue = $K->array($value);
                // If the value is undetermined, copy the value to newValue.
                // If the value is determined, update the value to newValue.
                if($this->undetermined) {
                    // if the value is MaskedNDArray, copy the mask to newValue.
                    if($value instanceof MaskedNDArrayInterface) {
                        $this->value = new MaskedNDArray($this->copy($newValue),$value->mask());
                    } else {
                    ...
                    $source = $newValue;
                    if($source instanceof MaskedNDArrayInterface) {
                        $source = $source->value();
                    }
                    $K->update($target, $source);
                }
            }
