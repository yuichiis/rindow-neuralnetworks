<?php
namespace RindowTest\NeuralNetworks\Layer\Conv2DTest;
...
class Test extends TestCase
{
    ...
    public function testDefaultInitialize()
    {
        ...
        $inputs = $g->Variable($K->zeros([1,4,4,1]));
        $layer->build($inputs);
        $params = $layer->getParams();
        $this->assertCount(2,$params);
        $this->assertEquals([3,3,1,5],$params[0]->shape());
        $this->assertEquals([5],$params[1]->shape());
        $this->assertNotEquals($mo->zeros([3,3,1,5])->toArray(),$params[0]->toArray());
        $this->assertEquals($mo->zeros([5])->toArray(),$params[1]->toArray());

        $grads = $layer->getGrads();
        $this->assertCount(2,$grads);
        $this->assertEquals([3,3,1,5],$grads[0]->shape());
        $this->assertEquals([5],$grads[1]->shape());
        $this->assertEquals($mo->zeros([3,3,1,5])->toArray(),$grads[0]->toArray());
        $this->assertEquals($mo->zeros([5])->toArray(),$grads[1]->toArray());

        $this->assertEquals([2,2,5],$layer->outputShape());
    }
    ...
    public function testUnmatchSpecifiedInputShape()
    {
        ...
        $inputs = $g->Variable($K->zeros([1,4,4,2]));
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Input shape is inconsistent: defined as [4,4,1] but [4,4,2] given in Conv2D');
        $layer->build($inputs);
    }
