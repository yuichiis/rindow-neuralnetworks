<?php
namespace RindowTest\NeuralNetworks\Gradient\Func\ExpandDimsTest;
...
class ExpandDimsTest extends TestCase
{
    ...
    public function testMiddleAxisNormal()
    {
        ...
        $inputs = $g->Variable($K->zeros([2,3]));
        $copyInputs = $K->copy($inputs);
        $outputsVariable = $nn->with($tape=$g->GradientTape(),
            function() use ($g,$inputs) {
                $outputsVariable = $g->expandDims($inputs,1);
        ...
        $this->assertEquals([2,1,3],$outputs->shape());
    ...
    public function testLeftAxisNormal()
    {
        ...
        $outputsVariable = $nn->with($tape=$g->GradientTape(),
            function() use ($g,$inputs) {
                $outputsVariable = $g->expandDims($inputs,0);
                return $outputsVariable;
            }
        );
        $outputs = $K->ndarray($outputsVariable);
        //
        $this->assertEquals([2,3],$inputs->shape());
        $this->assertEquals([1,2,3],$outputs->shape());
        $this->assertEquals($copyInputs->toArray(),$inputs->toArray());
    ...
    public function testRightAxisNormal()
    {
        ...
        //
        $this->assertEquals([2,3],$inputs->shape());
        $this->assertEquals([2,3,1],$outputs->shape());
        $this->assertEquals($copyInputs->toArray(),$inputs->toArray());
