<?php
namespace RindowTest\NeuralNetworks\Gradient\Func\OnesTest;
...
class Test extends TestCase
{
    ...
    public function testArrayShape()
    {
        ...
        $func = $g->Function(
            function($x) use ($g){
                $y0 = $g->ones($g->shape($x),dtype:NDArray::float32);
                $y1 = $g->ones($g->shape($x),dtype:NDArray::int32);
                return [$y0,$y1];
            }
        );
        // build
    ...
    public function testPhpArray()
    {
        ...
        $func = $g->Function(
            function($a,$b,$c) use ($g){
                $y0 = $g->ones([1,$a,$b,$c],dtype:NDArray::float32);
                $y1 = $g->ones([1,$c,$b,$a],dtype:NDArray::int32);
                return [$y0,$y1];
            }
        );
        // build
