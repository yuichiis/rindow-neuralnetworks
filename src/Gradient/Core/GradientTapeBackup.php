...
class GradientTape implements Context
{
    ...
    {
        $K = $this->backend;
        $funcs = [spl_object_id($target->creator())=>$target->creator()];
        while(count($funcs)) {
            ...
            foreach($dDatas as $dData) {
                ...
                if($creator) {
                    if(!array_key_exists(spl_object_id($creator),$funcs)) {
                        $funcs[spl_object_id($creator)] = $creator;
