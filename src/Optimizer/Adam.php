...
*/
    ...
    public function update(array $params, array $grads) : void
    {
        ...
        $K->update_increment($this->iter, 1.0);
        $t = $this->iter->toArray(); // Number of steps t
        $t = (int)floor($t);
        ...
        // PyTorch includes safeguards to prevent division by very small values,
        // but I will start by trying out a basic implementation.
        ...
        foreach(array_map(null, $params, $grads, $this->m, $this->v) as [$p, $g, $m, $v]) {
            ...
            // 2. Calculate bias-corrected moments. (m_hat, v_hat)
            // m_hat = m / (1 - beta1^t)
